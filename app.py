...
app.config['SQLALCHEMY_DATABASE_URI']='mysql+mysqlconnector://root:@localhost/sti data'
...
@app.route('/update_congenital_syphilis', methods=['POST'])
def update_congenital_syphilis():
    msg = ""
    client_id = request.form['unique_id']
    ...
    if 'loggedin' in session:
        if client_id:
            ...
            with engine.connect() as con:
                result_report = con.execute(text(f"SELECT * FROM congenital_syphilis WHERE unique_id = '{client_id}'"))
            ...
            if report_data:
                ...
                # Handling checkbox input (non-trep tests)
                non_trep_tests = request.form.getlist('non-trep-test')
                non_trep_tests_str = ','.join(non_trep_tests)  # Store as a comma-separated string
                
                # More fields to capture and process
                ...
                msg = "Congenital syphilis report updated successfully"
                return render_template('home.html', msg=msg)
            else:
                # Redirect to the home page if report not found
